...
//        populate_origin((MultiAdapter) adapter);
    ...
    public static abstract class MultiAdapter extends AbsAdapter {
        ...
        public abstract CameraPosition getCameraPositions(int page);
...
//                }
                Marker _marker = allMarkers.get(page);
//                map.removeMarker(_marker);
                _marker.remove();
            }
            map.clear();
    ...
    private void populateMulti(final MultiAdapter adapter) {
        ...
        for (int page = 0; page < adapter.getCount(); page++) {
            if (adapter.getCameraPositions(page) != null) {
                CameraPosition cp = adapter.getCameraPositions(page);
                if (cp != null) {
                    double lat = cp.target.latitude;
                    double lon = cp.target.longitude;
                    MarkerOptions mvo = createMarkerOptions(cp, adapter.getMarkerTitle(page, 1), "unselect");
                    Marker marker = map.addMarker(mvo);
                    allMarkers.add(marker);
//                    allOptions.add(mvo);
//                      mv.setInfoWindowAnchor(0.5f, 0.1f);
                } //else allOptions.add(null);
            }
        }
//        map.addMarkerViews(allOptions);
//        allMarkers = map.getMarkers();
...
//        map.getMarkerViewManager().setOnMarkerViewClickListener(createMarkerClickListenerMulti(adapter));
    ...
    private GoogleMap.OnMarkerClickListener createMarkerClickListenerMulti(final MultiAdapter adapter1) {
        return new GoogleMap.OnMarkerClickListener() {
            @Override
            public boolean onMarkerClick(Marker marker) {
                CameraPosition select =
                        new CameraPosition.Builder().target(new LatLng(marker.getPosition().latitude, marker.getPosition().longitude))
                                .zoom(13.5f)
                                .bearing(0)
                                .tilt(0)
                                .build();
...
//                        for (int i = 0; i < adapter1.getCameraPositions(page).size(); i++) {
                        CameraPosition cp = adapter1.getCameraPositions(page);
...
//    }
    ...
    private Marker updateMarks(final MultiAdapter adapter, CameraPosition select_cp, String title){
        ...
        for(int page = 0; page < allMarkers.size(); page++){
            ...
            if (select_cp.target.latitude == _marker.getPosition().latitude
                    && select_cp.target.longitude == _marker.getPosition().longitude
...
//            map.updateMarker(res);
    ...
    GoogleMap.OnCameraIdleListener onMoveEnd = new GoogleMap.OnCameraIdleListener() {
        @Override
            ...
            if(isSwipingBar){
                ...
                CameraPosition select =
                    new CameraPosition.Builder().target(
                            new LatLng(_adapter.getCameraPositions(nSwipingId).target.latitude,
                                    _adapter.getCameraPositions(nSwipingId).target.longitude))
                            .zoom(14.0f)
                            .bearing(0)
                            .tilt(0)
                            .build();
...
//                    populate();
                    select_location = new LatLng(_adapter.getCameraPositions(nSwipingId).target.latitude,_adapter.getCameraPositions(nSwipingId).target.longitude);
                    for (int i = 0 ; i < GD.customerModel.bars.size() ; i++) {
                        ...
                        double calc_distance = distance(select_location.latitude, select_location.longitude, lat, lon) ;
                        if (calc_distance <=  1) {
...
//        if (allMarkers.get(page).size() == 1)
            return CameraUpdateFactory.newCameraPosition(adapter.getCameraPositions(page));
...
//        return CameraUpdateFactory.newLatLngBounds(bounds, mapOffset);
    ...
    private MarkerOptions createMarkerOptions(CameraPosition cp, String title, String select_flag) {
...
//    }
    ...
    private BitmapDescriptor resizeBitmap(int resId, String select_flag){
        if (select_flag.equals("select")) {
            newWidth = 200;
            newHeight = 200;
        } else if (select_flag.equals("unselect")){
...
//                marker.setAlpha(markersAlpha);
    ...
    public Marker getMarker(int page, int position) {
        if (allMarkers.get(page) != null) return allMarkers.get(page);
        return null;
    }
