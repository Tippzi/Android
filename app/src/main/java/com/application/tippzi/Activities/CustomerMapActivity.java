...
public class CustomerMapActivity extends FragmentActivity implements MapViewPager.Callback  {
...
//        mapFragment.getMapAsync(this);
        ...
        for (int i = 0; i < GD.customerModel.walletModels.size(); i ++) {
            if (GD.customerModel.walletModels.get(i).claim_check == true) {
                ...
                if (!walletListModel.category.equals(GD.category_option)) {continue;}
                wallet_count++;
            }
        }
        if (wallet_count == 0) {
        ...
        //ui controls and category title
